...
namespace organizer
{
    public partial class OrganizerForm : Form
    {
         ...
         * Статистика
         */
        private void statsWindowCreator(int statsType)
        {
            int count = 0;
            ...
            /**
             * Если требуется количество задач на сегодняшний день
             */
            if (statsType == 0)
            {
                count = getCountOfTasks(start, end);
            }
            ...
            if ( statsType == 1 )
            {
                string[,]  dowArr = 
                    new string[,]{
                        ...
                        {"saturday", "суббота"}, 
                        {"sunday", "воскресенье"}};
                string dow = Convert.ToString(DateTime.Today.DayOfWeek).ToLower();
                int dnow = 0;
                ...
                start = start.Date.AddDays(-1 * dnow);
            ...
            /**
             * Статистика за эту месяц.
             */
            if ( statsType == 2 )
            {
                start = Convert.ToDateTime("01." + Convert.ToString(DateTime.Today.Date.Month) + "." + Convert.ToString(DateTime.Today.Date.Year));
                end = Convert.ToDateTime(Convert.ToString(DateTime.DaysInMonth(Convert.ToInt32(DateTime.Today.Date.Year), Convert.ToInt32(DateTime.Today.Date.Month))) + "." + Convert.ToString(DateTime.Today.Date.Month) + "." + Convert.ToString(DateTime.Today.Date.Year));
                count = getCountOfTasks(start, end);
            }


            /**
             * Статистика за эту год.
             */
            if ( statsType == 3 )
            {
                start = Convert.ToDateTime("01.01." + Convert.ToString(DateTime.Today.Date.Year));
                end = Convert.ToDateTime("31.12." + Convert.ToString(DateTime.Today.Date.Year));
                count = getCountOfTasks(start, end);
            }

            MessageBox.Show("Общее количество задач равно " + count);
        }

        /**
         * Функция возвращяет количество задач за указанный период времени.
         * Используем для статистики.
         * Указываем начальную и конечную дату.
         */
        public int getCountOfTasks( DateTime start, DateTime end )
        {
            int count = 0;
            if ( start == end )
            {
                foreach ( ArrayList al in todoList )
                {
                    if ( DateTime.Compare(Convert.ToDateTime(al[0]).Date, DateTime.Now.Date) == 0 )
                    {
                        count++;
                    }
                }
                foreach ( ArrayList al in doneList )
                {
                    if ( DateTime.Compare(Convert.ToDateTime(al[0]).Date, DateTime.Now.Date) == 0 )
                    {
                        count++;
            ...
            else
            {
                foreach ( ArrayList al in todoList )
                {
                    if ( ( DateTime.Compare(Convert.ToDateTime(al[0]).Date, start.Date) >= 0 ) && ( DateTime.Compare(Convert.ToDateTime(al[0]).Date, end.Date) <= 0 ) )
                    {
                        count++;
                    }
                }
        ...
        private void OrganizerForm_Paint( object sender, PaintEventArgs e )
        {
            repainListView( DateTime.Today );
            getNextTask( );
        }
        ...
        {
            statsWindowCreator(0);
        }
        private void наНеделюToolStripMenuItem_Click( object sender, EventArgs e )
        {
            statsWindowCreator(1);
        }

        private void наМесяцToolStripMenuItem_Click( object sender, EventArgs e )
        {
            statsWindowCreator(2);
        }

        private void наГодToolStripMenuItem_Click( object sender, EventArgs e )
        {
            statsWindowCreator(3);
        }
